...
internal sealed partial class Hub : PanelContainer, IDisposable
{
    ...
    private void OnServerInfoUpdated(string key, NL.ServerInfo info)
    {
    ...
    private void OnServerUpdated(string key, NL.Server server)
    {
        if (_rows.TryGetValue(key, out var row))
        {
    ...
    private void AddRow(string currentHostName, string key, NL.Server server)
    {
