...
public sealed partial class TimerTriggerComponent : Component
{
    ...
    /// </summary>
    [DataField, AutoNetworkedField] public List<string> KeysIn = new() { TriggerSystem.DefaultTriggerKey };

    /// <summary>
    /// The key that will trigger once the timer is finished.
    /// </summary>
    [DataField, AutoNetworkedField] public string? KeyOut = "timer";

    /// <summary>
    /// The time after which this timer will trigger after it is activated.
    /// </summary>
    [DataField, AutoNetworkedField] public TimeSpan Delay = TimeSpan.FromSeconds(1);

    /// <summary>
    /// If not empty, a user can use verbs to configure the delay to one of these options.
    /// </summary>
    [DataField, AutoNetworkedField] public List<TimeSpan> DelayOptions = new();
    ...
    /// Not networked because it's only used server side.
    /// </remarks>
    [DataField] public TimeSpan? InitialBeepDelay;

    /// <summary>
    /// The time between beeps.
    /// </summary>
    [DataField, AutoNetworkedField] public TimeSpan BeepInterval = TimeSpan.FromSeconds(1);
    ...
    /// </summary>
    [DataField, AutoNetworkedField] public SoundSpecifier? BeepSound;

    /// <summary>
    /// Whether you can examine the item to see its timer or not.
    /// </summary>
    [DataField, AutoNetworkedField] public bool Examinable = true;

    /// <summary>
    /// If true, the timer's next activation will be blocked.
    /// </summary>
    [DataField] public bool Disabled = false;

    /// <summary>
    /// The popup to show the user when starting the timer, if any.
    /// </summary>
    [DataField, AutoNetworkedField] public LocId? Popup = "timer-trigger-activated";
