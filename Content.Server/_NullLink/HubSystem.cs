...
public sealed partial class HubSystem : EntitySystem, IServerObserver, IServerInfoObserver
{
    ...
    private ConcurrentDictionary<string, NL.Server> _serverData = [];
    private ConcurrentDictionary<string, NL.ServerInfo> _serverInfoData = [];
    private readonly ConcurrentQueue<NL.UpdateEvent> _updateEvents = [];
    ...
    public override void Initialize()
    {
        ...
        SubscribeNetworkEvent<NL.Subscribe>(OnSubscribe);
        SubscribeNetworkEvent<NL.Resubscribe>(OnResubscribe);
        SubscribeNetworkEvent<NL.Unsubscribe>(OnUnsubscribe);
    ...
    private void OnSubscribe(NL.Subscribe _, EntitySessionEventArgs args)
    {
        _subscriptions[args.SenderSession] = _timing.RealTime;
        RaiseNetworkEvent(new NL.ServerData
        {
    ...
    private void OnResubscribe(NL.Resubscribe _, EntitySessionEventArgs args)
        => _subscriptions[args.SenderSession] = _timing.RealTime;
    private void OnUnsubscribe(NL.Unsubscribe _, EntitySessionEventArgs args)
    ...
    public Task Updated(string key, NLServer value)
    {
        ...
        _updateEvents.Enqueue(new NL.AddOrUpdateServer
        {
            Key = key,
            Server = Map(value)
        });
    ...
    public Task Remove(string key)
    {
        ...
        _updateEvents.Enqueue(new NL.RemoveServer
        {
            Key = key
        });
        return Task.CompletedTask;
    ...
    public Task Updated(string key, NLServerInfo value)
    {
        ...
        _updateEvents.Enqueue(new NL.AddOrUpdateServerInfo
        {
            Key = key,
    ...
    private static NL.Server Map(NLServer server)
        => new()
        {
            Title = server.Title,
            Description = server.Description,
            Type = (NL.ServerType)server.Type,
            IsAdultOnly = server.IsAdultOnly,
            ConnectionString = server.ConnectionString
        };
    private static NL.ServerInfo Map(NLServerInfo info)
        => new()
        {
            Status = (NL.ServerStatus)info.Status,
